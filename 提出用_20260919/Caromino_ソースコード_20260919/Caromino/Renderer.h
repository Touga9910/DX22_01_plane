...
struct LIGHT
{
	...
	BOOL Dummy[3];							//構造体のサイズ調整のため（各型は、自分のサイズの倍数に先頭アドレスを置きたがる）
	DirectX::SimpleMath::Vector4 Direction;	//平行光源の方向
	DirectX::SimpleMath::Color Diffuse;		//平行光源の強さと色
	DirectX::SimpleMath::Color Ambient;		//環境行の強さと色
};
...
struct MATERIAL
{
	...
	DirectX::SimpleMath::Color Diffuse;		//拡散反射（ノットイコールカラー）
...
private:
	...
	static ID3D11Buffer*			m_pLightBuffer;	//平行光源追加時に追記
	static ID3D11Buffer*			m_pMaterialBuffer;	//平行光源追加時に追記
...
public:
	...
	static void SetLight(LIGHT Light);			//平行光源追加時に追記
	static void SetMaterial(MATERIAL Material);	//マテリアル追加時に追記

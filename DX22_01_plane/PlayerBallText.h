...
namespace PlayerBallText
{
	...
	inline const char* GetDescription(const std::string& definitionId)
	{
		...
		if (definitionId == "player_pierce") return Utf8(u8"敵を貫通する生成球。実際に貫いた直線軌道を射線として残し、後続球に利用させる。");
		if (definitionId == "player_bounce") return Utf8(u8"壁反射が得意な生成球。強い反射で壁ごとの蓄積を増やし、後続球の反射を強化する。");
		...
		if (definitionId == "player_trace_driver") return Utf8(u8"既存の貫通射線に沿ってから壁で曲げ、曲がった先の実軌道を新しい射線として残す。");
		if (definitionId == "player_pierce_finisher") return Utf8(u8"貫通射線を利用すると決着ダメージを得る。同じショットで異なる敵を貫くほど強くなる。");
		if (definitionId == "player_ricochet_finisher") return Utf8(u8"蓄積された壁スタックを一打で何度も消費でき、反射を重ねるほど命中ダメージが上がる。");
	...
	inline std::string GetStats(const PlayerBallData& ball, const BallStatus& status)
	{
		...
		if (ball.definitionId == "player_cushion_charge")
			...
			text << Utf8(u8"射線耐久 ") << status.traceDurability << Utf8(u8" / 必要並走距離 ") << status.traceUseDistance;
		else if (ball.definitionId == "player_pierce_finisher")
			text << Utf8(u8"射線利用ボーナス ") << status.pierceFinisherBaseBonus << Utf8(u8" / 追加対象ごと +") << status.pierceFinisherMultiTargetBonus;
		else if (ball.definitionId == "player_ricochet_finisher")
			text << Utf8(u8"壁スタック利用ごと攻撃 +") << status.ricochetFinisherBonusPerUse << Utf8(u8" / 反発 ") << status.restitution;

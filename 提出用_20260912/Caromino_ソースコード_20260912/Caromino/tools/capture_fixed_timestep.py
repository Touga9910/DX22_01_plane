...
def window_for(process):
    ...
    def visit(hwnd, _):
        ...
        if pid.value == process.pid and name.value == "Caromino_WindowClass":
...
def capture(args):
    exe = Path(args.exe).resolve() if args.exe else (BASELINE / "Caromino.exe" if args.baseline else
           PROJECT.parent / "x64/Debug/Caromino.exe")
    results = []
    for case, ball, enemies, direction, power in CASES:
        ...
        env = os.environ.copy()
        if args.render_hz:
            env["Caromino_TEST_RENDER_HZ"] = str(args.render_hz)
